...
describe('ValidateCoupon', () => {
  ...
  it('caps fixed discount at subtotal', async () => {
    const uc = new ValidateCoupon(new FakeCouponRepo(makeCoupon({ type: 'fixed', value: 50_000 })))
  ...
  it('throws NotFound when coupon missing', async () => {
    ...
    await expect(uc.execute({ code: 'NOPE', subtotalMinor: 100 })).rejects.toBeInstanceOf(NotFoundError)
  })

  it('rejects when subtotal below minimum', async () => {
    const uc = new ValidateCoupon(new FakeCouponRepo(makeCoupon({ minSubtotalMinor: 5_000 })))
    await expect(uc.execute({ code: 'SAVE10', subtotalMinor: 1_000 })).rejects.toBeInstanceOf(
  ...
  it('rejects when redemption limit reached', async () => {
    ...
    await expect(uc.execute({ code: 'SAVE10', subtotalMinor: 10_000 })).rejects.toBeInstanceOf(
      PreconditionFailedError,
    )
  })
})
