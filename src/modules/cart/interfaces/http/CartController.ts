...
import {
  AddToCartInput,
  type AddToCart,
} from "../../application/use-cases/AddToCart";
import {
  ApplyCouponInput,
  type ApplyCoupon,
} from "../../application/use-cases/ApplyCoupon";
...
export class CartController {
  ...
  add = async (req: Request, res: Response): Promise<void> => {
    ...
    const guestToken =
      typeof req.body.guestToken === "string"
        ? req.body.guestToken
        : (req.header("x-guest-token") ?? null);
  ...
  applyCouponHandler = async (req: Request, res: Response): Promise<void> => {
    const input = ApplyCouponInput.parse({
    ...
    res
      .status(200)
      .json(ok({ cart: result.cart, discountMinor: result.discountMinor }));
  };
}
